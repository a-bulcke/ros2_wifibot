...
namespace wifibot
{
  ...
  {
    //    int16_t sl = (left * _loopControlSpeed * _ticsPerMeter) / 1000.0;
    //    int16_t sr = (right * _loopControlSpeed * _ticsPerMeter) / 1000.0;
    // Seems the firmware do the loop control speed conversion
    int16_t sl = (left * 50 * _ticsPerMeter) / 1000.0;
    int16_t sr = (right * 50 * _ticsPerMeter) / 1000.0;
    BYTE c = 0;

    sl = _between((int16_t)-W_SPEED_MAX, sl, (int16_t)W_SPEED_MAX);
    sr = _between((int16_t)-W_SPEED_MAX, sr, (int16_t)W_SPEED_MAX);
  ...
  c += 0x20;                // [00100000b] = control loop for right wheel
    ...
    uint16_t crc = _crc(_pBufferIn + 1, _pBufferIn[1] - 1);
    _pBufferIn[7] = crc >> 0;
    _pBufferIn[8] = crc >> 8;
  ...
  if (_pBuffer[b] == 0xff)
    ...
    pFrame[j] = _pBuffer[(b + j) % _bufferSize];
      ...
      uint16_t rcrc = (pFrame[_frameSize - 1] << 8) + pFrame[_frameSize - 2];
      uint16_t ccrc = _crc(pFrame + 1, _frameSize - 3);
